object Solution {
    def summaryRanges(nums: Array[Int]): List[String] = {
        ...
        while (i < nums.length) {
            ...
            // reached the end
            if (i == nums.length - 1) {
                println(num, rangeStart, rangeEnd)
                if (rangeStart == num ) {
                    result = result :+ rangeStart.toString
                } else {
                    result = result :+ (rangeStart.toString + "->" + rangeEnd.toString)
...
println(Solution.summaryRanges(Array(-2,-1,1,2,2147483646,2147483647))) // List("-2->-1","1->2","2147483646->2147483647")
println(Solution.summaryRanges(Array(-2,-1,1,2,7,8))) // List("-2->-1","1->2","2147483646->2147483647")
